...
final class AgentConversationViewModel: ObservableObject {
    ...
    private func trinityResponse(for text: String) -> String {
        ...
        if lower.contains("deploy") {
            return "I'll walk you through deploying your smart contract. Before deployment, I'll:\n\n1. **Security Scan** — Run automated analysis for common vulnerabilities\n2. **Gas Estimation** — Calculate deployment cost on your target network\n3. **Testnet First** — Deploy to a testnet so you can verify behavior\n4. **Constructor Args** — Help you set the right initialization parameters\n\nThis is a permanent, irreversible action on the blockchain. Which network would you like to deploy to?"
        }
    ...
    private func neoResponse(for text: String) -> String {
        ...
        if lower.contains("deploy") || lower.contains("update") || lower.contains("upgrade") {
            return "Ready for deployment operation. Pipeline status:\n\n1. Code compilation — standing by\n2. Security analysis — will run pre-deployment\n3. Testnet validation — prepared\n4. Mainnet deployment — awaiting your authorization\n\nProvide the deployment target and I will execute. Full audit trail will be maintained."
        }
